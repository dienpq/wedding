(function ($) {
  $.fn.letItSnow = function (options = {}) {
    ...
    const DEFAULTS = {
      background: null,
      quantity: "smooth",
      flake: { html: "&#x2022;", minSize: 1, maxSize: 7 },
    ...
    if (setup.background)
      container.css({ background: setup.background, backgroundSize: "cover" });
    let amount = 0;
    ...
    function initSnow() {
      ...
      container.find(`[data-flake]`).animate({ opacity: 1 }, 2e3);
      moveSnow();
    }
    function moveSnow() {
      const sizes = getContainerSizes();
      for (let i = 0; i <= amount; i++) {
        ...
        flake.css({ left: left + "px", top: data.posY + "px" });
